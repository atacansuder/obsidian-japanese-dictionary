...
const JMDICT_FORMS_TABLE_SYMBOL_TITLES: Record<string, string> = {
	"\u2605": "high priority form",
...
export function getJmdictFormsTableSymbolTitle(symbol: string): string {
	const title = JMDICT_FORMS_TABLE_SYMBOL_TITLES[symbol];
	if (title) return title;

	const titles = symbol
		.split("|")
		.map((part) => JMDICT_FORMS_TABLE_SYMBOL_TITLES[part.trim()])
